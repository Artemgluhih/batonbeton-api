...
// ✅ ИСПРАВЛЕНИЕ 1 & 2: Использование переменных окружения для всех чувствительных данных
const token = process.env.TELEGRAM_BOT_TOKEN;
const API_URL = process.env.API_URL || 'https://batonbeton-api.onrender.com';
const API_SECRET = process.env.API_SECRET;

// ✅ ИСПРАВЛЕНИЕ 6: Настройка timeout для axios запросов
...
// ✅ ИСПРАВЛЕНИЕ 8: Rate limiting - отслеживание запросов пользователей
...
// ✅ ИСПРАВЛЕНИЕ 3: Правильная валидация даты (не только формат)
...
// ✅ ИСПРАВЛЕНИЕ 7: Детальные сообщения об ошибках (в логи) и безопасные сообщения пользователю
...
// ✅ ИСПРАВЛЕНИЕ 5: Замена bot.once() на более безопасный конверсейшн флоу
...
async function handleBlockDate(chatId, date) {
  try {
    ...
    if (!isValidDate(date)) {
      bot.sendMessage(chatId, '❌ Неверный формат даты. Используйте: ДД-ММ-ГГГГ (например, 15-03-2025)');
      return;
    }
    
    // ✅ ИСПРАВЛЕНИЕ 4: Санитизация входных данных перед отправкой
    const sanitizedDate = date.replace(/[^0-9\-]/g, '');
    
    // ✅ ИСПРАВЛЕНИЕ 10: Передача секрета через заголовки (более безопасно, чем в теле)
...
async function handleUnblockDate(chatId, date) {
  try {
    ...
    const response = await axiosInstance.delete(
      `${API_URL}/api/admin/unblock-date`,
      {
